...
class Inode:
    def __init__(self, name, size, location):
        ...
        self.blocks = []  # 文件数据块的索引列表
        self.parent = None # 父目录

# Directory类，代表文件系统中的目录
class Directory:
...
class IndexedFileSystem:
    ...
    def create_directory(self, dir_name):
        """创建新目录"""
        new_dir = Directory(name=dir_name, location=self.get_current_path() + "/" + dir_name,
        ...
        for part in path_parts:
            if part == "..":
                ...
                if next_dir:
                    ...
                    print(f"目录 '{part}' 未找到.")#f"Directory '{part}' not found."
                    return

        self.current_directory = current_dir
        print(f" 已将目录更改为: {self.get_current_path()}")#Changed directory to
    ...
    def allocate_file(self, file_name, file_data, file_type="rw"):
        ...
        if free_blocks is None:
            print("没有足够的可用空间来分配文件.") # "Not enough free space to allocate the file."
            return
        ...
        print(f"文件 '{file_name}' 分配了内存块: {inode.blocks}")#allocated with blocks:

    def read_file(self, file_name):
        """读取文件"""
        if file_name not in self.current_directory.files:
            print(f"文件 '{file_name}' 未找到.")#not found
            return None
    ...
    def write_file(self, file_name, new_data):
        ...
        print(f"文件 '{file_name}' 写入新数据. Blocks: {inode.blocks}") #written with new data
    ...
    def delete_directory(self, dir_name):
        ...
        print(f"目录 '{dir_name}' 其内容已删除.")#and its contents deleted
    ...
    def copy_file(self, source_path, dest_path):
        ...
        new_file_name = source_file if dest_dir != source_dir else self.generate_new_name(source_file,
    ...
    def copy_directory(self, source_dir, dest_dir, new_name=None):
        """复制目录"""
        if new_name is None:
            new_name = source_dir.name if dest_dir != source_dir.parent else self.generate_new_name(source_dir.name,
                                                                                                    dest_dir.subdirectories)
    ...
    def recursive_copy_directory(self, src_dir, dst_dir):
        for file_name, file_inode in src_dir.files.items():
            ...
            new_file_inode = Inode(name=new_file_name, size=file_inode.size,
                                   location=dst_dir.location + "/" + new_file_name)
        ...
        for subdir_name, subdir in src_dir.subdirectories.items():
            new_subdir_name = subdir_name if subdir_name not in dst_dir.subdirectories else self.generate_new_name(
                subdir_name, dst_dir.subdirectories)
            new_subdir = Directory(name=new_subdir_name, location=dst_dir.location + "/" + new_subdir_name,
                                   parent=dst_dir)
    ...
    def change_file_type(self, file_name, new_type):
        ...
        inode.type = new_type
        inode.revise_time = datetime.now()  # 更新修改时间
        print(f"文件 '{file_name}' 类型已更改为 {new_type}.")#type changed to
